(ns warifu.cells.social-post.state-machine
  "Phase state machine for the 割符 — split tally, zero-fee card payment settlement
  social_post cell — the publication membrane that lets the actor self-publish observations
  to the mesh/AT-proto WITHOUT a server-held key. ADR-2606272355 (actor self-publication
  seed). Auto-seeded R0 (mirrors danjo.cells.social-post.state-machine / keizu's social_post
  membrane).
